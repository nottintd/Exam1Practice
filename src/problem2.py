...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for _ in range(n - 1):
        rect = rg.Rectangle(rg.Point(rect.get_upper_right_corner().x + delta, rect.get_upper_right_corner().y - delta),
                            rg.Point(rect.get_lower_left_corner().x - delta, rect.get_lower_left_corner().y + delta))
        rect.attach_to(win)
    win.render()
