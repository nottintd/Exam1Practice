...
def problem3a(window, point, n):
    ...
    window.render()
    totalthick = thickness
    for k in range(n - 1):
        i = k + 1
        line = rg.Line(rg.Point(point.x + (i * 20), point.y + (i * 10)),
                       rg.Point(end.x + (i * 20), end.y + (i * 10)))
        ...
        totalthick = totalthick + thickness
        line.attach_to(window)
        window.render()
    return totalthick
...
def problem3b(m, point1):
    ...
    for k in range(m):
        ...
        total = total + problem3a(window, point2, 3 + (k * 2))
